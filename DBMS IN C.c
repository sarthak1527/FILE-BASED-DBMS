...
#include <stdlib.h>
#include <string.h>
// This is done to have OS compatibility 
#ifdef _WIN32
    #include <direct.h>
...
typedef struct {
    char name[50];
    int columnCount;
    Column *columns; // Dynamic Allocation
} Table;
...
void useDatabase(char *name) {
    char path[300];
    sprintf(path, "%s%s%s", basePath, PATH_SEP, name);
    MKDIR(basePath); // ensure base exists
...
void createTable(char *name, char *columnsStr) 
{
    ...
    while (tok) {
        ...
        strcpy(tbl.columns[idx].dataType, "TEXT"); // simplified
        idx++;
        tok = strtok(NULL, ",");
    }

    // Save table file with header
    char filePath[300];
...
void deleteWhere(char *table, char *column, char *value) {
    ...
    char line[500];
    char header[500];
    fgets(header, sizeof(header), fp); // first row = column names
    ...
    while (tok) { cols[colCount] = tok; if (strcmp(tok, column)==0) colIdx=colCount; colCount++; tok=strtok(NULL,","); }
    ...
    while (fgets(line, sizeof(line), fp)) {
        ...
        int i=0;
        tok = strtok(line,",");
        while (tok) { vals[i++]=tok; tok=strtok(NULL,","); }
        if (colIdx!=-1 && strcmp(vals[colIdx], value)==0) { deleted++; continue; }
...
void dropTable(char *table) {
    char filePath[300];
    sprintf(filePath, "%s%s%s.txt", currentDB, PATH_SEP, table);
    if (remove(filePath) == 0) printf("Table '%s' dropped.\n", table);
    else printf("Table not found.\n");
}
